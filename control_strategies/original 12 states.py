...
def rotBE(r,p,y):
    ...
    rotBErow1 = horzcat((cosTE(y)*cosTE(p)), (sinTE(y)*cosTE(p)), (-sinTE(p)),0,0,0)
    rotBErow2 = horzcat((cosTE(y)*sinTE(p) *sinTE(r) - sinTE(y)*cosTE(r)), (sinTE(y)*sinTE(p) * sinTE(r) + cosTE(y)*cosTE(r)),(cosTE(p)*sinTE(r)),0,0,0)
    rotBErow3 = horzcat((cosTE(y)*sinTE(p) * cosTE(r) + sinTE(y)*sinTE(r)), (sinTE(y)*sinTE(p) * cosTE(r) - cosTE(y)*sinTE(r)), (cosTE(p)*cosTE(r)),0,0,0)
...
targetvel = np.array([[0.5],[0.0],[0.8]])
...
f_sim= vertcat(
    # 1
(T2*sinTE(theta2) - T4*sinTE(theta4))/m,
    # 2
(T1*sinTE(theta1) - T3*sinTE(theta3))/m,
    # 3
(T1*cosTE(theta1) + T2*cosTE(theta2) + T3*cosTE(theta3) + T4*cosTE(theta4))/m,
    # 4
((T2*cosTE(theta2)*arm_length) - (T4*cosTE(theta4)*arm_length) + (Iyy*dpitch*dy - Izz*dpitch*dy))/Ixx,
    # 5
(T1*cosTE(theta1)*arm_length - T3*cosTE(theta3)*arm_length + (-Ixx*droll*dy + Izz*droll*dy))/Iyy,
    # 6
(T1*sinTE(theta1)*arm_length + T2*sinTE(theta2)*arm_length + T3*sinTE(theta3)*arm_length + T4*sinTE(theta4)*arm_length + (Ixx*droll*dpitch - Iyy*droll*dpitch))/Izz,
)
...
f_linear_acc_sim = vertcat(((rotEB(roll, pitch, yaw)@(f_sim))[0:3]))
